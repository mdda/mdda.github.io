...
had its own 'winters' since Neural Networks were first fashionable in the 1980s.
...
(i) Rectified Linear Units (ReLU) layers 
and (ii) informed intialisation schemes.  

ReLU layers, where an output vector $y$ is given by $y=max(Wx+b, 0)$, simply 
avoid scaling problems by having the non-linearity agnostic to scale.

Initialising the weights to random values that are scaled 
based on fan-in/fan-out considerations is a way of making the 
...
Neural Networks roughly as quickly as gradient descent, should have been 
telling : With exact gradients, local search should have been much more effective
than any global search method.
